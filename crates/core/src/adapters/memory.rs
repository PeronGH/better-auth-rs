...
mod tests {
    ...
    async fn create_and_get_user() {
        let db = adapter();
        let user = db
            .create_user(CreateUser::new().with_email("test@test.com").with_name("Test"))
            .await
            .unwrap();
    ...
    async fn update_user() {
        let db = adapter();
        let user = db
            .create_user(CreateUser::new().with_email("update@test.com").with_name("Old"))
            .await
            .unwrap();
    ...
    async fn create_and_get_verification() {
        ...
        let found = db.get_verification("test-id", "secret-value").await.unwrap();
        assert!(found.is_some());
