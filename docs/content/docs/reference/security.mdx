...
Better Auth includes built-in request protection that matches the TypeScript implementation:

- cookie-backed mutating requests validate `Origin` or `Referer`
- first-login form flows use Fetch Metadata checks plus trusted-origin validation
...
- [ ] Configure trusted origins and keep request protection enabled
