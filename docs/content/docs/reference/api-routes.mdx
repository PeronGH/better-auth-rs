...
| POST | `/verify-password` | Yes | Verify the current user's password |
...
| GET | `/account-info` | Yes | Get provider account info for a linked account |
...
| GET | `/admin/get-user` | Admin | Get a user by id |
| POST | `/admin/create-user` | Admin | Create a new user |
| POST | `/admin/update-user` | Admin | Update a user |
...
| GET | `/passkey/generate-authenticate-options` | No | Get WebAuthn authentication options |
...
**Admin** endpoints additionally require the current user to satisfy the admin permission check for that route. With the default config, the built-in `admin` role is authorized and `user` is not.
