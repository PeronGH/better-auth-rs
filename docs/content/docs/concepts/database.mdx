...
for auth persistence through the companion `better-auth-seaorm` crate.

The supported public path is:

1. Define auth entities in your app with `#[derive(AuthEntity)]` from `better-auth-seaorm`.
2. Register them in an app schema with `#[derive(AuthSchema)]`.
3. Manage auth migrations in your own SeaORM migrator.
4. Build a `SeaOrmStore` from the shared `DatabaseConnection` and pass it to `BetterAuth::<YourSchema>::new(config).store(...)`.
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let store = SeaOrmStore::<AppAuthSchema>::new(Arc::new(config.clone()), auth_database);
